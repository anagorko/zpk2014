...
/*
 * Obiekty klasy Stos moga przechowywac dowolne obiekty implementujace interfejs
 * ElementStosu.
 */
...
class Stos
{
    ...
    Link* wierzcholek;
    int numer; // rozwiazanie problemu zliczania elementow
...
{
    Link* nowy = new Link;
    nowy -> poprzedni = wierzcholek; // wskazniki
    nowy -> dane = el;
    wierzcholek = nowy;
    numer++;
}
...
{
    if(wierzcholek == NULL) //poczatek stosu
        return NULL;
...
// program wyswietla stos, ale nie wykonuje obliczen
int main()
{
    Stos s;
