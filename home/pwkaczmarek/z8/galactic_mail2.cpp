...
int main(int, char**)
{
    ...
    while (true)
    {
        ...
        if (coll > 1) {
                string text = "Udalo Ci sie przezyc " + to_string(static_cast<int>(how_long)) + " sekund!";
                const char * text2 = text.c_str();
                al_show_native_message_box(al_get_current_display(),
                                 "Koniec gry!",
                                 "Niestety Twoj statek ulegl zniszczeniu.",
                                 //static_cast<char const *>(text),
                                  text2,
                                 NULL, NULL);
                break;
         }
    ...
    cout << "Twoja gra trwala " << static_cast<int>(how_long) << " sekund(y)." << endl;
    return 0;
}
