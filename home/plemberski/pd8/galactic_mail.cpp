...
public:
    Flying()
    {
        bitmap = al_load_bitmap("asteroid.png");//ładowanie obrazka
    }
    ...
    virtual void move(float time) {
        Vect p = getPosition();
        p += getVelocity() * time;
        if (p.getX() > screen_w) { p.setX(p.getX() - screen_w); }//jesli obiekt "wyjdzie"poza ekran na prawo to "wraca" z lewej
...
//prawda gdy obiekty sie sderzaja
...
//al_init_image_addon() - inicjalizacja doddawania bitmap

    ALLEGRO_DISPLAY *display = al_create_display(screen_w, screen_h);//otwarcie okna allegro z zadanymi wymiarami
    ALLEGRO_TIMER *timer = al_create_timer(1.0 / FPS); //szybkość odswieżania obrazu
    ALLEGRO_EVENT_QUEUE *event_queue = al_create_event_queue(); //kolejka zdarzen z klawiatury, myszki.
    ...
    al_start_timer(timer); //Start the timer specified. From then, the timer’s counter will increment at a constant rate, and it will
//begin generating events. Starting a timer that is already started does nothing.
...
// tworzymy 5 asteroid
    ...
    al_stop_timer(timer); //Stop the timer specified. The timer’s counter will stop incrementing
    int64_t trwanie = al_get_timer_count(timer)/60;//Return the timer’s counter value
    cout << "GAME OVER. Czas trwania gry: "  << trwanie << " sekund!" << endl;
