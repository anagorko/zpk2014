...
string komunikat1;
    ...
    while (true)
    {
        ...
        if(ev.type == ALLEGRO_EVENT_TIMER) {
            ...
            if (time2 > 15.0) {
                create_asteroid(); time2 = 0.0;
            }
        } else if(ev.type == ALLEGRO_EVENT_DISPLAY_CLOSE)
        { komunikat1 = "Dziękuję za grę!";
            break;
        ...
        if (coll > 1)
        { komunikat1 = "Zderzyłeś sie z asteroidą!!!";
            break;
        }
...
//Okno z czasem gry
time (& koniec);
int czasGry = difftime( koniec, start );

stringstream sstm;
sstm << komunikat1 << endl << "Grałeś przez " << czasGry << " sek."; // konwersja int na string
string komunikat2 = sstm.str();

const char *cstr = komunikat2.c_str(); // konwersja stringa na char

al_show_native_message_box(display, "Galactic Game", cstr , "", "", 1);
