...
const int screen_w = sz*k_sz;   // szerokoœæ ekranu (screen width)
const int screen_h = wy*k_wy;    // wysokoœæ ekranu (screen height)
...
/********************************************************************************
 * Tu rozpoczyna siê istotna czêœæ kodu                                         *
 ********************************************************************************/
...
enum typ_kafelka
{
    ...
    // potrzebne do deklaracji tablicy poni¿ej
...
struct kafelek
{
    ...
    int kto; // wykorzystujemy to pole, by pamiêtaæ, który gracz postawi³ bombê
    // pole wykorzystywane przez bomby i wybuch
    int dl; // ile pozosta³o jeszcze czasu do wybuchu / do wypalenia
};
...
const string plik_z_kafelkiem[liczba_kafelkow] =
{
    ...
    "kafelki/sciana_niezniszczalna.png", // œciana niezniszczalna
    "kafelki/sciana.png", // œciana zwykla
...
bool przygotuj_bitmapy()
{
    for (int i = 0; i < liczba_kafelkow; i++) {
        ...
        if (!bitmapa[i]) {
            cerr << "B³¹d podczas wczytywania bitmapy " << plik_z_kafelkiem[i] << "." << endl;
            return false;
        }
    }
...
void przygotuj_plansze()
{
    ...
    //
    // obok graczy musi byæ pusta przestrzeñ
    //
    ...
    //
    // plansza otoczona jest niezniszczalnymi œcianami
    //
    ...
    p.g[0].kolor = 0;
    p.g[0].wariant = 0;
    p.g[0].energia = 100; // zaczynamy gotowi do dzia³ania :)
    p.g[0].ile_bomb = 1;
...
void rysuj_plansze()
{
    ...
    for (int i = 0; i < sz; i++) {
        for (int j = 0; j < wy; j++) {
            ...
            // póŸniej œciany - mog¹ przykrywaæ obiekty
    ...
    //
    // wyœwietlamy zasiêg i iloœæ bomb ka¿dego z graczy
    //
...
void aktualizuj_plansze()
{
    ...
    //
    // skanujemy planszê w poszukiwaniu wybuchów
    //
    for (int i = 0; i < sz; i++) {
        for (int j = 0; j < wy; j++) {
    ...
    //
    // skanujemy planszê w poszukiwaniu bomb
    //
        ...
        for (int j = 0; j < wy; j++) {
            if (p.obiekt[i][j].typ == bomba) {
                ...
                if (p.obiekt[i][j].dl <= 0) {
                    ...
                    // gracz mo¿e j¹ znów stawiaæ
                    p.g[kto].ile_bomb++;

                    int z = p.g[kto].zasieg_wybuchu;
                    int k;

                    // propagacja wybuchu w górê
                    k = 0;
                    while (k <= z) {
...
/**************************************************************************
 * To koñczy siê istotna czêœæ kodu. Kod poni¿ej jest w miarê generyczny  *
 **************************************************************************/
...
int main(int argc, char ** argv)
{
    ...
    //
    // Event loop - g³ówna pêtla programu
    //
    ...
    while(!wyjdz)
    {
        ...
        if(ev.type == ALLEGRO_EVENT_TIMER) {
            //
            // minê³a 1/60 (1/FPS) czêœæ sekundy
            //
            p.g[0].energia++;
            p.g[1].energia++;
            przerysuj = true;
