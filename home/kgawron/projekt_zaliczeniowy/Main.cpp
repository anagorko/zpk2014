...
int display_height = block_height * vertical_blocks + gap_size * ( vertical_blocks + 1 );
// Blocks values
...
bool prepare_bitmaps()
{
    ...
    for( int i = 0; i < 4; ++i )
    {
        result[ i ] = al_load_bitmap( result_files[ i ].c_str() );
        if( !result[ i ] )
        {
            cerr << "Prepare Bitmaps error." << endl;
            cerr << "Could not load: " << result_files[ i ] << endl;
            return false;
        }
...
{
    return block_height * row + gap_size * (row + 1);
}
...
// drawing game result (one block at a time)
// specific image in specific column and row
void draw_one_result_block( int index, int column, int row )
{
    al_draw_bitmap( result[ index ],
                   get_block_x( column ),
                   get_block_y( row ),
                   0);
}
...
{
    draw_one_result_block( 0, getRandom( 0, 1 ), getRandom( 0, 1 ) );
    draw_one_result_block( 1, getRandom( 2, 3 ), getRandom( 0, 1 ) );
    if( result == "lost")
    {
        draw_one_result_block( 2, getRandom(0, 3), getRandom( 2, 3 ) );
    }
    else
    {
        draw_one_result_block( 3, getRandom( 0, 3 ), getRandom( 2, 3 ) );
    }
}
