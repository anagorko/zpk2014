...
protected:
    ...
    Vect p, v; //wsp. srodka obiektu, predkosc piksele/sek
    float a, r; //kat jak statek, kometa jest obrocony, predkosc kontowa
...
public:
    ...
    virtual void move(float time) {
        Vect p = getPosition();
        p += getVelocity() * time; //s=v * t
        if (p.getX() > screen_w) { p.setX(p.getX() - screen_w); }//przerzucaj o szerokosz ekranu w lewo
    ...
    }//prawda gdy obiekty sie zderzaja,  diameter - œrednica, na zasadzie ko³a z uwglednieniem srodkow
...
public:
    ...
    virtual void move(float time) {
        ...
        Flying::move(time);//wywo³ujemy tamten move
    }
};
...
vector<Flying*> objects;//tablica do wskaznikow obiektow klasy flying. nazwa objects
...
void create_asteroid()
{
    ...
    do {
        a -> setPosition(Vect(rand() % screen_w, rand() % screen_h));//rand ca³kowita liczba losowa
    } while (a -> getPosition().distanceTo(objects[0] -> getPosition()) < 200);
//objects[0] to statek!! Byle nie by³oby bli¿ej ni¿ 200 piksweli
    a -> setAngle(rand());
    a -> setRotation(((float) (rand() % 200) - 100) / 100.0);//od -1 do1
...
int main(int, char**)
{
    ...
    //funkcje
    ALLEGRO_DISPLAY *display = al_create_display(screen_w, screen_h); //tworzymy okienko
    ALLEGRO_TIMER *timer = al_create_timer(1.0 / FPS); //jak szybko odswieza siê obraz
    ALLEGRO_EVENT_QUEUE *event_queue = al_create_event_queue(); //tworzy kolejke zdarzen z klawiatury, myszki itp.

    if (display == NULL || timer == NULL || event_queue == NULL) {
        cout << "B³¹d inicjalizacji." << endl;
        return 2;
    }
    //zrodla zdarzen z klawiatury itp.
    al_register_event_source(event_queue, al_get_display_event_source(display));
    al_register_event_source(event_queue, al_get_timer_event_source(timer));
    al_register_event_source(event_queue, al_get_keyboard_event_source());

    al_clear_to_color(al_map_rgb(0,0,0));
    al_flip_display(); //koloruje

    al_start_timer(timer); //wystartowanie zegarka

    Spaceship *s = new Spaceship(); //tworzymy wskazniki do klasy spaceship
    s -> setPosition(Vect(screen_w / 2, screen_h / 2));
    s -> setAngle(0);
    s -> setRotation(0);
    s -> setVelocity(Vect(0, 0)); //ustawienie parametrow poczatkowych

    objects.push_back(s);//dok³adany pierwszy element do tablicy

    for (int i = 0; i < 5; i++) {
        create_asteroid();
    }

    for (Flying* f: objects) {//dla wszystkich obiektów rób to co w pêtli i zapisz w f?
        if (f -> getBitmap() == NULL) {
    ...
    while (true)
    {
        ...
        if (coll > 1) { break; }//statek zawsze bze sob¹ koliduje
        ...
        for (Flying* f: objects) {
            ...
            al_draw_rotated_bitmap(f -> getBitmap(),
                                   ...
                                   f -> getAngle(),
                                   0);
//po co tyle kopii? Aby ³adnie siê przenosi³o na drug¹ stronê ekranu
        }
        al_flip_display();
    };
    ...
    cout << "Przegrales!! Twoj wynik to: "  << setprecision(3) << life_time << " sekund!" << endl;
