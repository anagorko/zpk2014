...
int main(int, char**)
{
    ...
    while (true)
    {
        ...
        if (coll > 1) {
            ...
            al_init_font_addon();
            al_init_ttf_addon();
            ALLEGRO_FONT *font_koniec = al_load_ttf_font("MyUnderwood.ttf",50,0 );
            ALLEGRO_FONT *font_czas = al_load_ttf_font("MyUnderwood.ttf",40,0 );
            if (!font_koniec || !font_czas){
                fprintf(stderr, "Could not load 'MyUnderwood.ttf'.\n");
                return -1;
            }

            al_clear_to_color(al_map_rgb(0,0,0));
            al_draw_text(font_koniec, al_map_rgb(255,255,255), screen_w/2, (screen_h/4),ALLEGRO_ALIGN_CENTRE, "Koniec gry!");
            al_draw_textf(font_czas, al_map_rgb(255,255,255), screen_w/2, (screen_h/2.5),ALLEGRO_ALIGN_CENTRE, "Czas gry: %.2f s", (double)czas);
