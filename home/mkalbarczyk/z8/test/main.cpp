...
int main(int, char**)
{
    ...
    while (true)
    {
        ...
        if (coll > 1) {
                ...
                ALLEGRO_FONT *font_over = al_load_font("orbitron-black.ttf", 36, NULL);
                ALLEGRO_FONT *font_time = al_load_font("orbitron-black.ttf", 18, NULL);

                if(!font_over||!font_time){
                    cout<<"Could not load font.";
                }
                al_draw_text(font_over, al_map_rgb(255,255, 255), screen_h/2, screen_w/4, ALLEGRO_ALIGN_CENTER, "GAME OVER");
                al_draw_textf(font_time, al_map_rgb(255,255, 255), screen_h/2, screen_w/3, ALLEGRO_ALIGN_CENTER, "Your game lasted: %.2f s", (double)time2);

                al_flip_display();
                al_rest(5.0);
                al_destroy_font(font_over);
